...
config.read(os.getcwd()+"/govcrawler/config.ini")
...
class Gov_Table_Url(Base):
    ...
    def sele_state(self):
        try:
            ...
            session = DBSession()
            user = session.query(Gov_Table_Url).filter(Gov_Table_Url.state == "0").all()
            return user
        except:
            print("提取采集状态出错")
    ...
    def sele_by_url(self, url):
        try:
            DBSession = sessionmaker(bind=engine)
            session = DBSession()
            user = session.query(Gov_Table_Url).filter(Gov_Table_Url.url == url, Gov_Table_Url.state==1).first()
            session.close()
            return user
        except:
            print(url+"网站采集异常")

class Gov_Table(Base):
